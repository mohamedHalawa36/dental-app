...
const CORS_ALLOWED_ORIGINS = (Deno.env.get('CORS_ALLOWED_ORIGINS') || '').split(',').map((s)=>s.trim()).filter(Boolean);
...
Deno.serve(async (req)=>{
  try {
    ...
    const { data: tokenUser, error: tokenError } = await svc.auth.getUser(accessToken);
    ...
    const { data: profileData, error: profileError } = await svc.from('profiles').insert(profileRow).select().single();
    if (profileError) {
      // Attempt to rollback user creation if profile insert fails
      console.error('Profile insert failed, attempting to delete created auth user', profileError.message);
      await svc.auth.admin.deleteUser(newUser.id).catch((e)=>console.error('Rollback delete failed', e));
      return new Response(JSON.stringify({
