...
const CORS_ALLOWED_ORIGINS = (Deno.env.get("CORS_ALLOWED_ORIGINS") || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean); // optional env var
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: targetProfile, error: targetProfileError } = await svcClient
      .from("profiles")
      .select("is_admin")
      .eq("id", targetUserId)
      .single();
    ...
    const { data: deletedData, error: deleteError } =
      await svcClient.auth.admin.deleteUser(targetUserId);
    if (deleteError) {
      return new Response(
        JSON.stringify({
    ...
    // After auth user deleted, soft delete their profile row if it exists
    const { error: softDeleteError, status } = await svcClient
      .from("profiles")
      .update({
    ...
    if (softDeleteError) {
      return new Response(
        ...
        { status: status || 500, headers: buildCorsHeaders(req) },
      );
    }
    //Success response
    return new Response(
